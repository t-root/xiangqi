...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, const bool cutNode) {
        ...
        && (cutNode == (ttData.value >= beta) || depth > 5))
    {
        ...
        // Also skip when a check-streak house rule is active for the mover (see
        // Position::check_streak_active): that counter isn't in the Zobrist key either, so a
        // cached score here could belong to a different streak count reached via another
        // move order.
        if (pos.rule60_count() < 116 && !pos.check_streak_active())
        {
