...
void UCIEngine::command(std::string command) {
    // WebAssembly invokes one complete UCI command at a time. Reuse the normal UCI loop with
    // a temporary one-shot command line so the browser build and native build share all rules.
    const int   savedArgc = cli.argc;
    char**      savedArgv = cli.argv;
    char        program[] = "pikafish";
    char*       argv[] = {program, command.data()};
