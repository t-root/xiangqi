...
u64 parse_check_streak_root(const std::string& text) {
    ...
    for (size_t pos = 0; pos < text.size();)
    {
        ...
        if (part.size() < 4 || part[2] != ':')
            continue;
        if (part[0] < 'a' || part[0] > 'i' || part[1] < '0' || part[1] > '9')
            continue;
...
Engine::Engine(std::optional<std::filesystem::path> path) :
    ...
    // House rules ported from the app's own "Engine Web" (see position.h/.cpp
    // CheckStreak namespace, Position::rule_judge). Off by default ("none") so stock UCI
    // clients that don't know about this see stock Pikafish behaviour, at stock speed.
    // "both" restricts BOTH sides — the app's "Duyệt & Điều Khiển" rule (limit 3: a 4th
    // consecutive check by the same piece always loses, even if mate).
    // "white"/"black" restricts only that side — the app's asymmetric analysis rule
    // (first player unrestricted; second player limited to 2).
    options.add(  //
      "CheckStreak_RestrictedColor", Option("none", [](const Option& o) {
          std::string v                  = std::string(o);
          CheckStreak::restricted[WHITE] = (v == "white" || v == "both");
          CheckStreak::restricted[BLACK] = (v == "black" || v == "both");
          return std::nullopt;
      }));
    ...
    // Root check-streak already in progress mid-game (see CheckStreak::root in position.h) — a
    // comma separated list of "square:count" in plain UCI text ("e4:2,c7:1"), "none" clears it.
    // Applied to the position in Engine::set_position, right after Position::set() resets it to 0.
    options.add(  //
      "CheckStreak_RootRed", Option("none", [](const Option& o) {
          CheckStreak::root[WHITE] = parse_check_streak_root(std::string(o));
          return std::nullopt;
      }));

    options.add(  //
      "CheckStreak_RootBlack", Option("none", [](const Option& o) {
          CheckStreak::root[BLACK] = parse_check_streak_root(std::string(o));
          return std::nullopt;
      }));
...
std::optional<PositionSetError> Engine::set_position(const std::string&              fen,
    ...
    // Stamp the root check-streak the app told us about (see CheckStreak::root in position.h) —
    // Position::set() just reset it to 0, which is only correct for a genuinely fresh position, not
    // one mid-game with a streak already in progress.
    states->back().checkStreak[WHITE] = CheckStreak::root[WHITE];
    states->back().checkStreak[BLACK] = CheckStreak::root[BLACK];
