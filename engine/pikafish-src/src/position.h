...
struct StateInfo {
    ...
    // House rule ported from the app's own "Engine Web": for each color, the set of its pieces
    // currently holding a check streak, with how many CONSECUTIVE own moves each of them has been
    // delivering check for. A "checker" here is a piece that genuinely attacks the enemy king
    // (st->checkersBB), NOT the piece that happened to move: a knight move that also uncovers a
    // rook check credits BOTH pieces, and a pure discovered check credits the rook alone.
    // Packed as base-CS_DIGIT_BASE digits, see the CheckStreak namespace below. 0 = no streak, and
    // only the colors CheckStreak::restricted[] marks are ever tracked (an unrestricted color has
    // no limit, so it stays at 0). Deliberately NOT folded into the Zobrist key (same reasoning as
    // rule60): see the rule60_count() < 116 TT-cutoff guard in search.cpp, mirrored for this via
    // Position::check_streak_active().
    u64 checkStreak[COLOR_NB];
...
// House rule config, set via UCI options (see engine.cpp) before each "go" — mirrors the app's
// own checkStreakRestrictedColor/checkStreakLimit. restricted[] tells which sides the limit
// applies to: neither (default) means the rule is off, matching stock Pikafish behaviour with
// zero overhead when unused; both entries true is the app's "Duyệt & Điều Khiển" rule, where each
// side gets the same limit.
//
// root[]: the app only ever sends the CURRENT board via "position fen <fen>" (no move history), so
// Position::set() always resets checkStreak to 0 — the search would otherwise have no idea a streak
// is already in progress mid-game. This mirrors the app's own liveCheckStreak (see
// xiangqi-analyzer.html csEntries/csAdvanceAfterMove) and gets stamped onto the root StateInfo right
...
class Position {
   public:
    ...
    bool  check_streak_active() const;
    u16   chased(Color c);
...
// Mirrors the rule60_count() < 116 TT-cutoff guard in search.cpp: a streak in progress for a
// restricted side (not folded into the Zobrist key) means a cached score here could belong to a
// different streak count reached via a different move order — must not be used for a TT cutoff.
inline bool Position::check_streak_active() const {
    return (CheckStreak::restricted[WHITE] && st->checkStreak[WHITE])
        || (CheckStreak::restricted[BLACK] && st->checkStreak[BLACK]);
}
