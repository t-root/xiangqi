...
// All false = rule off (default). Set through UCI options "CheckStreak_RestrictedColor"
// ("white" / "black" / "both" / "none") and "CheckStreak_Limit" — see engine.cpp — before "go",
// exactly like the app sets checkStreakRestrictedColor/checkStreakLimit before each analysis/game.
bool restricted[COLOR_NB] = {false, false};
int  limit                = 2;
...
*/
    ...
    // A freshly-set position (new "position fen ..." from the GUI) never carries an in-progress
    // check streak. Engine::set_position stamps the app's real state right after this call.
    st->checkStreak[WHITE] = st->checkStreak[BLACK] = 0;
...
void Position::do_move(Move                      m,
    ...
    // House rule: one and the same piece checking CheckStreak::limit+1 times in a row (for a
    // restricted side only) loses — see csAdvanceAfterMove() in the app's own engine, which this
    // mirrors step for step. Every piece that genuinely checks gets its own counter, so a double
    // check credits both pieces and a discovered check credits the uncovered piece, not the mover.
    // Must run after st->checkersBB above: that bitboard IS the set of pieces to credit.
    // "Still the same piece" is recognised by the square it stood on BEFORE this move: `from` for
    // the piece that just moved, its own square for every other checker (only one own piece can
    // have moved since this side's previous move). The verdict itself is left to rule_judge().
    if (!CheckStreak::restricted[us] || !givesCheck)
        st->checkStreak[us] = 0;
    else
    {
        u64      prev = st->checkStreak[us];
        u64      next = 0, mult = 1;
        int      slot = 0;
...
bool Position::rule_judge(Value& result, int ply) {
    ...
    // House rule: one of the mover's (~sideToMove) pieces has now been checking one time too many
    // in a row. Skip entirely when the rule is off or the mover isn't a restricted side — this
    // keeps stock behaviour, and stock speed, whenever the app hasn't turned the rule on.
